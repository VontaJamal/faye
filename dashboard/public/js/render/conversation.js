...
function contextMeta(message, displayCode) {
...
export async function refreshConversationContext(state, sessionId, apiRequest, displayCode) {
    ...
    try {
        const response = await apiRequest(`/v1/conversation/${encodeURIComponent(sessionId)}/context?limit=8&includePending=true`);
...
export function renderConversationPanel(state, health, apiRequest, displayCode) {
    ...
    const stateBadgeState = active.state === "ended"
        ? "bad"
