import { DEBUG_MODE, EVENT_LABELS } from "../state/store.js";
...
function eventTitle(type, displayCode) {
    if (DEBUG_MODE) {
        return type;
    }
    return EVENT_LABELS[type] ?? displayCode(type);
}
function listenerStatusSummary(status, displayCode) {
    const map = {
...
function eventSummary(event, displayCode) {
...
function renderEvent(payload, displayCode) {
...
export function bindEventStream(state, displayCode, onConnectionStatus) {
