...
function renderServiceSummary(state, health) {
...
function renderRuntimeStatus(state, runtime, roundTrip, metrics) {
...
function renderFirstSuccessChecklist(state, health) {
...
export function apiKeyReadyForActiveProfile(state) {
    return itemFromOnboarding(state.latestHealth, "api-key-ready")?.ok === true;
}
export function renderHealthPanels(state, health) {
