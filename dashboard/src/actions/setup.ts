...
function clearSetupValidationState(form: HTMLFormElement, summary: HTMLDivElement | null): void {
...
function renderSetupValidationState(
...
function validateSetupPayload(
  state: DashboardState,
  form: HTMLFormElement
): { payload: SetupPayload | null; errors: Partial<Record<SetupFieldName, string>> } {
...
function profileCard(profile: Profile, activeProfileId: string, deps: SetupActionDeps): HTMLElement {
...
export async function refreshProfiles(deps: SetupActionDeps): Promise<void> {
  const profileList = deps.state.elements.profileList;
  if (!profileList) {
    return;
  }

  const data = await deps.apiRequest<ProfilesResponse>("/v1/profiles");
...
export function bindSetupForm(deps: SetupActionDeps): void {
...
export function bindCreateProfileForm(deps: SetupActionDeps): void {
...
export function bindQuickActions(deps: QuickActionDeps): void {
  ...
  listenerRestart?.addEventListener("click", async () => {
    try {
      await deps.apiRequest<{ result: ServiceStatusResult }>("/v1/listener/restart", { method: "POST" });
  ...
  bridgeRestart?.addEventListener("click", async () => {
    try {
      await deps.apiRequest<{ result: ServiceStatusResult }>("/v1/bridge/restart", { method: "POST" });
