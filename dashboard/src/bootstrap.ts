...
import { createDashboardState, type HealthResponse } from "./state/store.js";
...
export async function bootstrap(): Promise<void> {
  ...
  const setRecoveryStatus = (message: string, error = false): void => {
  ...
  const refreshHealth = async (): Promise<void> => {
    ...
    const run = (async () => {
      const health = await apiRequest<HealthResponse>("/v1/health");
